...
class Crawler:
    ...
    @property  # 这部分，改着改着，没用了，姑且留着吧
    def finished(self):  # 是否爬取完成
        if self.work_queue.empty():
            return True
        else:
            return False

    def img_process(self, scr_list, pics_in, title, scr_path_):  # 后台进程
        crop_pictures(scr_list, pics_in)  # 将不必要的部分裁去
        paste_images(im_path=scr_path_ + title)  # 传入文件夹名称
        shutil.rmtree(scr_path_ + title)  # 删除文件夹
        self.work_queue.get()  # 取出一个队列中的对象
    ...
    def begin(self):
        ...
        if len(id_list) == 1:
            ...
            driver.quit()
            self.img_process(scr_list, pics_in, title, scr_path_)
        else:
            ...
            for i in range(len(id_list)):
                ...
                try:
                    if i == len(id_list) - 1:
                        img_process_thread = threading.Thread(target=self.img_process,
                                                              args=(scr_list, pics_in, title, scr_path_))  # 多线程优化
                        img_process_thread.start()
                        if work_queue:
                            work_queue.put(img_process_thread)
                        driver.quit()
                    else:
                        img_process_thread = threading.Thread(target=self.img_process,
                                                              args=(scr_list, pics_in, title, scr_path_))  # 多线程优化
                        ...
                        if time_all >= 40:  # 经过一段时间，百度文库的模式会更改为仅 VIP 可点击 “展开”  (PS: 这段时间是越来越短了啊)
                            driver.quit()
        ...
        while True:
            if work_queue.empty():  # 队列中的所有对象都被取了出来
                break  # 结束程序运行
